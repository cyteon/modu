...
> {
    let expr = recursive(|expr| {
        ...
        let array = select! { (Token::LBracket, span) => span }
            ...
            .map(
                |((start, elements), end): ((Span, Vec<SpannedExpr>), Span)| SpannedExpr {
        ...
        let object = select! { (Token::LBrace, start) => start }
            ...
            .map(
                |((start, entries), end): ((Span, Vec<(String, SpannedExpr)>), Span)| {
        ...
        let shift = unary
            ...
            .foldl(
                choice((
                    select! { (Token::BitShl, span) => span }
                        .then(unary.clone())
                        .map(|(span, right)| (Token::BitShl, span, right)),
                    select! { (Token::BitShr, span) => span }
                        .then(unary.clone())
                        .map(|(span, right)| (Token::BitShr, span, right)),
                ))
                .repeated(),
                |left: SpannedExpr, (op, _span, right): (Token, Span, SpannedExpr)| SpannedExpr {
                    node: match op {
                        Token::BitShl => {
                            Expr::BitShl(Box::new(left.clone()), Box::new(right.clone()))
                        }
                        Token::BitShr => {
                            Expr::BitShr(Box::new(left.clone()), Box::new(right.clone()))
                        }
                        _ => unreachable!(),
                    },
        ...
        let multiplicative = power
            ...
            .foldl(
                choice((
                    select! { (Token::Star, span) => span }
                        .then(power.clone())
                        .map(|(span, right)| (Token::Star, span, right)),
                    select! { (Token::Slash, span) => span }
                        .then(power.clone())
                        .map(|(span, right)| (Token::Slash, span, right)),
                    select! { (Token::Mod, span) => span }
                        .then(power.clone())
                        .map(|(span, right)| (Token::Mod, span, right)),
                ))
                .repeated(),
                |left: SpannedExpr, (op, _span, right): (Token, Span, SpannedExpr)| SpannedExpr {
                    node: match op {
        ...
        let additive = multiplicative
            ...
            .foldl(
                choice((
                    select! { (Token::Plus, span) => span }
                        .then(multiplicative.clone())
                        .map(|(span, right)| (Token::Plus, span, right)),
                    select! { (Token::Minus, span) => span }
                        .then(multiplicative.clone())
                        .map(|(span, right)| (Token::Minus, span, right)),
                ))
                .repeated(),
                |left: SpannedExpr, (op, _span, right): (Token, Span, SpannedExpr)| SpannedExpr {
                    node: match op {
        ...
        let range = additive
            ...
            .map(
                |(start, range): (SpannedExpr, Option<(Span, SpannedExpr)>)| match range {
        ...
        let comparison = inclusive_range
            .clone()
            .foldl(
                choice((
                    select! { (Token::Equal, span) => span }
                        .then(inclusive_range.clone())
                        .map(|(span, right)| (Token::Equal, span, right)),
                    select! { (Token::NotEqual, span) => span }
                        .then(inclusive_range.clone())
                        .map(|(span, right)| (Token::NotEqual, span, right)),
                    select! { (Token::LessThan, span) => span }
                        .then(inclusive_range.clone())
                        .map(|(span, right)| (Token::LessThan, span, right)),
                    select! { (Token::LessThanOrEqual, span) => span }
                        .then(inclusive_range.clone())
                        .map(|(span, right)| (Token::LessThanOrEqual, span, right)),
                    select! { (Token::GreaterThan, span) => span }
                        .then(inclusive_range.clone())
                        .map(|(span, right)| (Token::GreaterThan, span, right)),
                    select! { (Token::GreaterThanOrEqual, span) => span }
                        .then(inclusive_range.clone())
                        .map(|(span, right)| (Token::GreaterThanOrEqual, span, right)),
                    select! { (Token::In, span) => span }
                        .then(inclusive_range.clone())
                        .map(|(span, right)| (Token::In, span, right)),
                ...
                |left: SpannedExpr, (op, _span, right): (Token, Span, SpannedExpr)| SpannedExpr {
                    node: match op {
                        ...
                        Token::NotEqual => {
                            Expr::NotEqual(Box::new(left.clone()), Box::new(right.clone()))
                        }
                        Token::LessThan => {
                            Expr::LessThan(Box::new(left.clone()), Box::new(right.clone()))
                        }
                        Token::LessThanOrEqual => {
                            Expr::LessThanOrEqual(Box::new(left.clone()), Box::new(right.clone()))
                        }
                        Token::GreaterThan => {
                            Expr::GreaterThan(Box::new(left.clone()), Box::new(right.clone()))
                        }
    ...
    let stmt = recursive(|stmt| {
        let let_stmt = select! { (Token::Let, span) => span }
            ...
            .map(
                |(((start, name), value), end): (((Span, String), SpannedExpr), Span)| {
                    SpannedExpr {
                        node: Expr::Let {
                            name,
                            value: Box::new(value),
        ...
        let assign_stmt = expr
            ...
            .map(
                |(((target, op), value), end): (
                    ((SpannedExpr, Option<AssignOp>), SpannedExpr),
                    Span,
                )| {
        ...
        let expr_stmt = expr
            ...
            .then(select! { (Token::Semicolon, span) => span }.labelled("semicolon"))
            .map(
                |((expr, _), end): ((SpannedExpr, SimpleSpan), Span)| SpannedExpr {
                    node: expr.node,
        ...
        let block = select! { (Token::LBrace, span) => span }
            ...
            .map(
                |((start, stmts), end): ((Span, Vec<SpannedExpr>), Span)| SpannedExpr {
        ...
        let fn_stmt = select! { (Token::Function, span) => span }
            ...
            .map(
                |(((start, name), args), body): (((Span, String), Vec<String>), SpannedExpr)| {
                    SpannedExpr {
                        node: Expr::Function {
                            name,
                            args,
        ...
        let class_stmt = select! { (Token::Class, span) => span }
            ...
            .map(
                |(((start, name), parent), ((_lbrace, methods), end)): (
                    ((Span, String), Option<String>),
                    ((Span, Vec<SpannedExpr>), Span),
                )| SpannedExpr {
                    node: Expr::Class {
                        name,
                        methods,
        ...
        let for_loop_stmt = select! { (Token::For, span) => span }
            ...
            .then(block.clone().labelled("loop body"))
            .map(
                |(((start, iterator_name), iterator_range), body): (
                    ((Span, String), SpannedExpr),
                    SpannedExpr,
                )| {
                    SpannedExpr {
                        node: Expr::ForLoop {
        ...
        let while_loop_stmt = select! { (Token::While, span) => span }
            ...
            .then(block.clone().labelled("loop body"))
            .map(
                |((start, condition), body): ((Span, SpannedExpr), SpannedExpr)| SpannedExpr {
                    node: Expr::WhileLoop {
        ...
        let if_stmt = select! { (Token::If, span) => span }
            ...
            .map(
                |((((start, condition), then_branch), else_if_branches), else_branch): (
                    (
                        ((Span, SpannedExpr), SpannedExpr),
                        Vec<((Span, SpannedExpr), SpannedExpr)>,
                    ),
                    Option<(Span, SpannedExpr)>,
                )| {
                    ...
                    branches.extend(
                        else_if_branches
                            .into_iter()
                            .map(|((_, cond), block)| (Some(cond), block)),
                    );
        ...
        let return_stmt = select! { (Token::Return, span) => span }
            .then(
                expr.clone()
                    .or_not()
                    .labelled("an expression after 'return'"),
            )
            .then(select! { (Token::Semicolon, span) => span }.labelled("semicolon"))
            .map(
                |((start, value), end): ((Span, Option<SpannedExpr>), Span)| SpannedExpr {
                    node: Expr::Return(match value {
                        Some(v) => Box::new(v),
        ...
        let import_stmt = select! { (Token::Import, span) => span }
            ...
            .then(select! { (Token::Semicolon, span) => span }.labelled("semicolon"))
            .map(
                |(((start, name_expr), alias), end): (
                    ((Span, SpannedExpr), Option<(Span, String)>),
                    Span,
                )| {
        ...
        let try_catch_stmt = select! { (Token::Try, span) => span }
            ...
            .then(
                select! { (Token::Catch, span) => span }
                    .then(select! { (Token::Identifier(name), _) => name }.or_not())
                    .then(block.clone().labelled("catch block"))
                    .or_not(),
            )
            .map(
                |((start, try_block), catch): (
                    (Span, SpannedExpr),
                    Option<((Span, Option<String>), SpannedExpr)>,
                )| {
                    ...
                    chumsky::error::RichReason::ExpectedFound { expected, found } => {
                        let (found_str, error_span) = match found {
                            ...
                            None => (
                                ...
                                Span::from(input.len() - 1..input.len() - 1),
                            ),
                        };
