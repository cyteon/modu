    ...
    async function run() {
        try {
            ...
            if (code.includes('import "http"')) {
                output =
                    "Error running code: The HTTP client does currently not work on the web";
                runClicked = false;
                return;
            } else if (code.includes('import "ffi"')) {
                output =
                    "Error running code: Using FFI is not possible on the web";
                runClicked = false;
                return;
            } else if (code.includes('import "os"')) {
                output =
                    "Error running code: Using the OS module is not supported in the web IDE cause its running on the web";
                runClicked = false;
                return;
            }
...
<svelte:head>
    ...
    <meta
        name="description"
        content="Online IDE for Modu, run code without installing anything."
    />
</svelte:head>
...
<div class="flex flex-col w-full h-screen">
    <div class="w-full border-b border-bg1 py-1 px-6 flex">
        ...
        <p class="ml-2 mt-auto">
            {moduVersion ? `v${moduVersion}` : "loading..."}
        </p>
        ...
        <div class="ml-auto flex">
            <button
                class={`${runClicked ? "text-blue" : ""} mr-5`}
                on:click={run}
            >
    ...
    <div
        class="flex p-4 h-full space-y-4 flex-col md:flex-row md:space-x-4 md:space-y-0"
    >
        <div
            class="bg-bg1 w-full p-6 pt-4 h-full rounded-lg flex flex-col md:w-2/3 border border-bg2"
        >
            <h1 class="text-3xl font-bold">Input</h1>
            <div
                id="code"
                class="mt-4 p-1 bg-bg0_h h-full max-h-[83vh] rounded-lg border border-bg2"
            ></div>
        </div>

        <div
            class="bg-bg1 w-full p-6 pt-4 h-full rounded-lg flex flex-col md:w-1/3 border border-bg2"
        >
            <h1 class="text-3xl font-bold">Output</h1>
            <pre
                class="px-4 py-2 mt-4 text-xl break-words whitespace-pre-wrap bg-bg0_h border border-bg2 rounded-lg h-full overflow-y-auto">{@html output}</pre>
        </div>
    </div>
</div>
